...
// import { useQuery } from "@tanstack/react-query"; // Not used in this snippet
...
// import AirlineLogo from "@/components/AirlineLogo"; // Not used in this snippet directly
// import { type Airport } from "@shared/schema"; // Not used
...
import { allAirlines, getAirlinesForRegion /*, getAirlinesForAirport*/ } from "@/lib/airlineUtil"; // getAirlinesForAirport not used
import { directFlightExists /*, type AirlineData*/ } from "@/lib/flightData"; // AirlineData not used
...
const majorAirports = {
  ...
  "Europe": [
    ...
    // Western Europe (Ensure all these have latitude and longitude for your calculator)
    { code: "CDG", name: "Paris Charles de Gaulle", latitude: 49.0097, longitude: 2.5479 }, // Example, fill all
   ...
   "Asia": [
    ...
    { code: "CXR", name: "Nha Trang", latitude: 12.0000, longitude: 109.2167 }, // Cam Ranh Airport
    // Cambodia
  ...
  "South America": [
    ...
    { code: "VVI", name: "Santa Cruz", latitude: -17.6448, longitude: -63.1354 }, // Viru Viru Intl
    ...
    { code: "CNF", name: "Belo Horizonte", latitude: -19.6244, longitude: -43.9719 }, // Tancredo Neves Intl
  ...
  "Africa": [
    ...
    // West Africa (NBO is East Africa, moved it)
    { code: "LOS", name: "Lagos", latitude: 6.5774, longitude: 3.3211 },
    { code: "ACC", name: "Accra", latitude: 5.6052, longitude: -0.1668 },
    { code: "DKR", name: "Dakar", latitude: 14.7397, longitude: -17.4901 }, // Blaise Diagne Intl
    // East Africa
    { code: "NBO", name: "Nairobi", latitude: -1.3192, longitude: 36.9278 }, // Jomo Kenyatta Intl
...
const allAirportsFlat = Object.values(majorAirports).flat().sort((a, b) => a.code.localeCompare(b.code)); // Renamed to avoid conflict

// REMOVED the local 'airlines' const. 'allAirlines' from airlineUtil.ts will be used.
...
// const generateDateOptions = () => { // Not used in component directly for dropdowns
//   const dates = [];
...
//   return dates;
// };
// const dateOptions = generateDateOptions(); // Not used
...
interface EnhancedFlightDetails {
  ...
  departureDate: string; // This will be calculatedData.departureDateLocal
  departureTime: string; // This will be calculatedData.departureTimeLocal
  arrivalDate: string;   // This will be calculatedData.arrivalDateLocal
  arrivalTime: string;   // This will be calculatedData.arrivalTimeLocal
  flightNumber: string;
  airline: {
    id: string | number; // From allAirlines
    code: string;
    name: string;
    logo?: string;    // Optional, from allAirlines
    region?: string;  // Optional, from allAirlines
  };
  duration: string;
  cabin: string;
  distanceKm: number;
  calculatedData: { // This is the raw output from your calculator
...
const EnhancedFlightSelection = () => {
  ...
  const [filteredAirlines, setFilteredAirlines] = useState(allAirlines); // Uses imported allAirlines
  ...
  const resetAll = () => {
    ...
    setAirlineRegionFilter("All Regions"); setFlightData(null); setDepartureDate(new Date());
  };
  
  useEffect(() => {
    if (airlineRegionFilter === "All Regions") {
      setFilteredAirlines(allAirlines); // Uses imported allAirlines
    } else {
  ...
  useEffect(() => {
    if (departureAirport) {
      ...
      if (region) { setAirlineRegionFilter(region); }
    }
  }, [departureAirport]);
  ...
  const handleSearch = (search: string) => {
    ...
    let airlineResults = allAirlines.filter(airline => { // Uses imported allAirlines
    ...
    setSearchResults({ airports: airportResults, airlines: airlineResults });
  };
  ...
  const handleGenerateFlight = async () => {
    if (!departureAirport || !destinationAirport) {
      toast({ title: "Missing information", description: "Please select both departure and destination airports", variant: "destructive" });
      return;
    }
    if (departureAirport === destinationAirport) {
      toast({ title: "Invalid selection", description: "Departure and destination airports cannot be the same", variant: "destructive" });
      return;
    }
    if (!selectedAirline) {
      toast({ title: "Missing information", description: "Please select an airline", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      // Find the airline information using 'allAirlines' imported from airlineUtil.ts
      const airlineInfo = allAirlines.find(a => a.code === selectedAirline);
      
      if (!airlineInfo) {
        // More specific error message for easier debugging
        console.error("Selected airline code for lookup:", selectedAirline);
        console.error("Available airline codes in allAirlines (from airlineUtil):", allAirlines.map(a => a.code));
        throw new Error(`Airline information not found for code: ${selectedAirline}. Check if this airline exists in the list provided by "@/lib/airlineUtil".`);
      }
      ...
      if (!calculatedData.departureDateLocal || !calculatedData.arrivalTimeLocal || !calculatedData.arrivalDateLocal || !calculatedData.departureTimeLocal || !calculatedData.durationFormatted || calculatedData.distanceKm === undefined) {
      ...
      const enhancedFlightData: EnhancedFlightDetails = {
        ...
        departureDate: calculatedData.departureDateLocal, // From calculator
        departureTime: calculatedData.departureTimeLocal, // From calculator
        arrivalDate: calculatedData.arrivalDateLocal,     // From calculator
        arrivalTime: calculatedData.arrivalTimeLocal,     // From calculator
        flightNumber: generateFlightNumber(selectedAirline),
        airline: { // Populate with details from airlineInfo (from allAirlines)
          id: airlineInfo.id, // Assuming airlineInfo has 'id'
          code: airlineInfo.code,
          name: airlineInfo.name,
          logo: airlineInfo.logo,   // Optional: if airlineInfo has 'logo'
          region: airlineInfo.region // Optional: if airlineInfo has 'region'
        },
        duration: calculatedData.durationFormatted,
        cabin: selectedCabin,
        distanceKm: calculatedData.distanceKm,
        calculatedData // Store the raw calculator output
      };
      ...
      toast({ title: "Error", description: `Failed to generate flight: ${error instanceof Error ? error.message : "Unknown error"}`, variant: "destructive" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleContinue = () => {
    if (!flightData) {
      toast({ title: "No flight selected", description: "Please generate a flight first", variant: "destructive" });
      return;
    }
    
    // The departureDate (Date object from state) is used for the initial calculation.
    // flightData.departureDate (string) is the calculated local departure date.
    // For setFlightDetails, we might want to pass the original user-selected date (if needed by next steps)
    // or the calculated one. Let's assume the calculated one is what's relevant for context if it's already localized.
    // However, FlightContext's departureDate is usually the *intended* departure date.
    
    // Use the original selected date for the context's departureDate if that's the convention
    const contextDepartureDate = departureDate ? format(departureDate, "yyyy-MM-dd") : flightData.departureDate;

    setFlightDetails({
      departureAirport: flightData.departureAirport,
      arrivalAirport: flightData.arrivalAirport,
      departureDate: contextDepartureDate, // Or flightData.departureDate if that's preferred
      departureTime: flightData.departureTime, // This is already localized by calculator
      calculatedFlightData: flightData // Pass the whole enhanced data
    });
    
    const mockFlight = {
      id: 1, // Mock flight ID for this booking instance
      flightNumber: flightData.flightNumber,
      airline: { // Use details from flightData.airline (which came from allAirlines)
        id: flightData.airline.id,
        code: flightData.airline.code,
        name: flightData.airline.name,
        logo: flightData.airline.logo || "",   // Fallback if not present
        region: flightData.airline.region || "" // Fallback if not present
      },
      departure: {
        airport: {
          id: 1, // Mock airport ID
        ...
        time: flightData.departureTime // Already localized
      },
      arrival: {
        airport: {
          id: 2, // Mock airport ID
        ...
        time: flightData.arrivalTime // Already localized
      },
  ...
  // IMPORTANT: Ensure all airports in 'majorAirports' have 'latitude' and 'longitude'.
  // Your 'calculateEnhancedFlightDetails' function likely needs these.
  // I've added placeholders or actual values for some; please verify all.
            ...
            <div className="relative">
              <div className="flex">
                <Input
                  ...
                  placeholder={
                    ...
                    !destinationAirport ? "Search for destination airport..." : 
                    !selectedAirline ? "Search for airline..." :
                ...
                />
                {isSearching ? (
                  <Button variant="ghost" className="absolute right-2 top-1/2 transform -translate-y-1/2" onClick={() => setIsSearching(false)}>
                    <ArrowRight className="h-4 w-4" /> {/* Changed to X or similar for close might be better UX */}
                  </Button>
                ) : (
                  <Button variant="ghost" className="absolute right-2 top-1/2 transform -translate-y-1/2" onClick={() => setIsSearching(true)}>
              ...
              {isSearching && (
                <div className="absolute z-10 mt-1 w-full bg-background border border-border rounded-md shadow-lg">
                  <Command> {/* Removed CommandInput as Input is outside */}
                    <div className="flex justify-between items-center p-2 border-b border-border">
                      ...
                      <Button variant="ghost" size="sm" className="h-6 px-2" onClick={() => setIsSearching(false)}>
                        Close
                      </Button>
                    </div>
                    <CommandList>
          ...
          <CardContent className="p-4 md:p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              <div className="space-y-4">
                <div>
                  <label className="block text-foreground font-medium mb-2 text-sm">Departure Region</label>
                  <Select value={departureRegion} onValueChange={setCachedDepartureRegion}>
                    ...
                    <SelectContent>
                      ...
                      {Object.keys(majorAirports).map((region) => (<SelectItem key={region} value={region}>{region}</SelectItem>))}
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  <label className="block text-foreground font-medium mb-2 text-sm">Departure Airport</label>
                  <Select value={departureAirport} onValueChange={setCachedDepartureAirport} disabled={!departureRegion && departureRegion !== "All Regions"}>
                    <SelectTrigger className="w-full bg-background"><SelectValue placeholder="Select departure airport" /></SelectTrigger>
                    <SelectContent>
                      {departureRegion === "All Regions" || !departureRegion ? 
                        allAirportsList.map((airport) => (<SelectItem key={airport.code} value={airport.code}>{airport.code} - {airport.name}</SelectItem>))
                        : majorAirports[departureRegion as keyof typeof majorAirports]?.map((airport) => (<SelectItem key={airport.code} value={airport.code}>{airport.code} - {airport.name}</SelectItem>))
                      }
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  <label className="block text-foreground font-medium mb-2 text-sm">Departure Date</label>
                  <Popover open={isDepartureCalendarOpen} onOpenChange={setIsDepartureCalendarOpen}>
                    ...
                    <PopoverContent className="w-auto p-0">
                      <Calendar mode="single" selected={departureDate} onSelect={(date) => { setDepartureDate(date); setIsDepartureCalendarOpen(false); }} initialFocus disabled={(date) => date < new Date(new Date().setHours(0,0,0,0))}/>
                    </PopoverContent>
                  </Popover>
                </div>
                <div>
                  <label className="block text-foreground font-medium mb-2 text-sm">Departure Time</label>
                  <div className="flex space-x-2">
                    <div className="w-1/2">
                      <Select value={departureHour} onValueChange={setCachedDepartureHour}>
                        <SelectTrigger className="w-full bg-background"><SelectValue placeholder="Hour" /></SelectTrigger>
                    ...
                    <div className="w-1/2">
                      <Select value={departureMinute} onValueChange={setCachedDepartureMinute}>
                        <SelectTrigger className="w-full bg-background"><SelectValue placeholder="Minute" /></SelectTrigger>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-foreground font-medium mb-2 text-sm">Destination Region</label>
                ...
                <div>
                  <label className="block text-foreground font-medium mb-2 text-sm">Destination Airport</label>
                  <Select value={destinationAirport} onValueChange={setCachedDestinationAirport} disabled={!destinationRegion && destinationRegion !== "All Regions"}>
                    <SelectTrigger className="w-full bg-background"><SelectValue placeholder="Select destination airport" /></SelectTrigger>
                    <SelectContent>
                      {destinationRegion === "All Regions" || !destinationRegion ? 
                        allAirportsList.map((airport) => (<SelectItem key={airport.code} value={airport.code}>{airport.code} - {airport.name}</SelectItem>))
                        : majorAirports[destinationRegion as keyof typeof majorAirports]?.map((airport) => (<SelectItem key={airport.code} value={airport.code}>{airport.code} - {airport.name}</SelectItem>))
                      }
                    </SelectContent>
                  </Select>
                </div>
                {departureAirport && destinationAirport && !hasDirectFlight && (
                  <Alert variant="destructive" className="bg-destructive/10 text-destructive">
                    <AlertTriangle className="h-4 w-4" /> <AlertTitle>No Direct Flights Available</AlertTitle>
                    <AlertDescription>
                      No direct flights known between {departureAirport} and {destinationAirport}. A theoretical direct flight will be generated.
                    </AlertDescription>
                  </Alert>
                )}
                <div className="space-y-4"> {/* This was nested, might be a layout choice */}
                  <div>
                    <label className="block text-foreground font-medium mb-2 text-sm">Airline Region Filter (Optional)</label>
                    ...
                    {departureAirport && airlineRegionFilter !== getAirportRegion(departureAirport) && getAirportRegion(departureAirport) !== "Unknown" && (
                      <p className="text-xs text-muted-foreground mt-1">Tip: Airlines from {getAirportRegion(departureAirport)} are common for {departureAirport}.</p>
                    )}
                  </div>
                  <div>
                    <label className="block text-foreground font-medium mb-2 text-sm">Airline</label>
                    <Select value={selectedAirline} onValueChange={setCachedSelectedAirline}>
                      ...
                      <SelectContent>
                        {filteredAirlines.map((airline) => (<SelectItem key={airline.id} value={airline.code}>{airline.code} - {airline.name}</SelectItem>))}
                      </SelectContent>
                    </Select>
                  </div>
                </div>
                <div>
                  <label className="block text-foreground font-medium mb-2 text-sm">Cabin Class</label>
                  <Select value={selectedCabin} onValueChange={setCachedSelectedCabin}>
                    ...
                    <SelectContent>{cabinClasses.map((cabin) => (<SelectItem key={cabin.value} value={cabin.value}>{cabin.label}</SelectItem>))}</SelectContent>
                  </Select>
                </div>
              </div>
            </div>
            <div className="mt-6 flex justify-between items-center flex-wrap gap-4">
              <Button variant="outline" onClick={resetAll} className="text-muted-foreground hover:text-destructive border-muted-foreground">
                <RefreshCcw className="mr-2 h-4 w-4" /> Reset All
              </Button>
              <Button onClick={handleGenerateFlight} disabled={!departureAirport || !destinationAirport || !selectedAirline || isLoading} className="w-full md:w-auto">
            ...
            <CardContent className="p-4 md:p-6">
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="space-y-1 text-center">
                  ...
                  <div className="text-sm font-medium">
                    ...
                    {flightData.calculatedData?.dayChange && flightData.calculatedData.dayChange !== 0 && (
                      <span className="ml-1 text-xs text-primary">
                        {flightData.calculatedData.dayChange > 0 ? `+${flightData.calculatedData.dayChange}` : flightData.calculatedData.dayChange}
                      </span>
                    )}
                  </div>
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                <div className="bg-muted/40 rounded-md px-3 py-1.5 text-xs flex items-center">
                  <Clock className="h-3.5 w-3.5 mr-1.5" /> Flight Duration: {flightData.duration}
                </div>
                {flightData.calculatedData?.timezoneDifference && (
                  <div className="bg-muted/40 rounded-md px-3 py-1.5 text-xs flex items-center">
                    <Clock className="h-3.5 w-3.5 mr-1.5" /> Time Zone Diff: {flightData.calculatedData.timezoneDifference}
                  </div>
                )}
              </div>
              <div className="mt-6 flex justify-end">
                <Button onClick={handleContinue}>
                  Continue to Passenger Details <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
              </div>
            </CardContent>
          </Card>
